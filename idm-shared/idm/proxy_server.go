...
func (l *Launcher) handleProxyRequest(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(targetURL, "googlevideo.com") {
			...
			if job.AudioUrl != "" {
				if mergedURL := l.dashMergeAndServe(targetURL, job.AudioUrl, job); mergedURL != "" {
					...
					// Merge falhou — tentar transformar o 'n' e continuar
					// Passa job completo (com itag/height) para que ytdlpMergeAndServe,
					// se acionado como fallback dentro de transformYouTubeURL, possa
					// respeitar a qualidade selecionada no dropdown.
					log.Printf("[DASH] Merge falhou para job %s — tentando fallback yt-dlp\n", jobID)
					targetURL = l.transformYouTubeURL(targetURL, job.Referrer, job)
				}
			} else {
				// Sem AudioUrl: transformar apenas o parâmetro 'n'.
				// Passa job completo para que o fallback yt-dlp (quando goja falha)
				// use o itag/height selecionado em vez de bestvideo+bestaudio.
				transformed := l.transformYouTubeURL(targetURL, job.Referrer, job)
	...
	if isHLS && !isSegment && resp.StatusCode < 400 {
		...
		if readErr == nil {
			// ── Tentar merge completo em arquivo único ANTES de só reescrever
			// o manifest como texto ──────────────────────────────────────────
			// PROBLEMA: o IDM é lançado apenas via linha de comando (/d) — isso
			// não aciona o motor de download HLS nativo do IDM (que normalmente
			// só ativa via detecção de vídeo na GUI). Sem este merge, o IDM
			// simplesmente salvava o texto do manifest reescrito como um
			// arquivo .m3u8 literal — que só "funciona" em outro player (ex:
			// VLC) enquanto o bridge continuar rodando (os segmentos apontam
			// para 127.0.0.1). Não é um download real e offline.
			//
			// genericHLSMergeAndServe baixa todos os segmentos, descriptografa
			// se necessário (AES-128) e concatena em um único .mp4 — servido
			// através do mesmo mecanismo já usado pelo merge DASH do YouTube.
			// Retorna "" para streams ao vivo (sem fim definido) ou em caso de
			// erro — nesses casos, cai no fallback abaixo (manifest reescrito).
			if mergedURL := l.genericHLSMergeAndServe(string(body), targetURL, job); mergedURL != "" {
				log.Printf("[HLS-MERGE] Job %s: manifest convertido em arquivo único → %s\n", jobID, mergedURL)
				mergedResp, mergedErr := l.doProxyFetch(r.Method, mergedURL, job, false, r.Header.Get("Range"))
				if mergedErr == nil {
					defer mergedResp.Body.Close()
					l.streamResponse(w, mergedResp, job, mergedURL)
					return
				}
				log.Printf("[HLS-MERGE] Falha ao servir arquivo mergeado via loopback: %v — fallback para manifest\n", mergedErr)
			}

			// Fallback: servir o manifest reescrito como texto (comportamento
			// anterior — usado para streams ao vivo ou quando o merge falha).
